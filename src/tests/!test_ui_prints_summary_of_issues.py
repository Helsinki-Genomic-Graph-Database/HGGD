...
class TestUI(unittest.TestCase):
    ...
    def setUp(self):
        ...
        creator = DatasetCreator(["src/tests/testdata_with_description_for_graph"], self.spdx_service)
        dataset_list = creator.get_datasets()
        stubio = StubIO()
        ui = UI(dataset_list, stubio, self.spdx_service)
        ui.start()
...
class TestUIGraphLicences(unittest.TestCase):
    ...
    def setUp(self):
        self.spdx_service = SpdxService()
        creator = DatasetCreator(["src/tests/testdata_with_no_licence_for_all_graphs"], self.spdx_service)
        dataset_list = creator.get_datasets()
        stubio = StubIO([""])
        ui = UI(dataset_list, stubio, self.spdx_service)
        ui.start()
...
class TestUIGraphLicencesWithLiceneGivenInUI(unittest.TestCase):
    ...
    def setUp(self):
        self.spdx_service = SpdxService()
        creator = DatasetCreator(["src/tests/testdata"], self.spdx_service)
    ...
    def test_ui_should_not_notify_if_licence_is_added_in_ui(self):
        stubio = StubIO(["test name", "short", "long", "MIT"])
        ui = UI(self.dataset_list, stubio, self.spdx_service)
        ui.start()
        res = stubio.outputs
...
class TestUINotificationsWithLogOrNot(unittest.TestCase):
    ...
    def setUp(self):
        ...
        creator = DatasetCreator([self.dir], self.spdx_service)
        self.dataset_list = creator.get_datasets()
